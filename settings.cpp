...
static int setting_dump_string(const TCHAR *service_name, void *param, const TCHAR *name, const value_t *value, const TCHAR *additional) {
  ...
  unsigned long type = (unsigned long)(uintptr_t) param;
...
static int setting_dump_exit_action(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  while (true) {
    ...
    const TCHAR *additional = (code[i]) ? code : NSSM_DEFAULT_STRING;
...
int dump_setting(const TCHAR *service_name, HKEY key, SC_HANDLE service_handle, settings_t *setting) {
  ...
  if (ret != 1) return ret;
  return setting_dump_string(service_name, (void *)(uintptr_t) setting->type, setting->name, &value, 0);
}
...
settings_t settings[] = {
  ...
  { NSSM_NATIVE_DESCRIPTION, REG_SZ, (void *) _T(""), true, 0, native_set_description, native_get_description, 0 },
  ...
  { NSSM_NATIVE_OBJECTNAME, REG_SZ, (void *) NSSM_LOCALSYSTEM_ACCOUNT, true, 0, native_set_objectname, native_get_objectname, native_dump_objectname },
